...
DEFAULT_GEMINI_PROMPT = (
    ...
    "    - 元記事が日本語の場合：本文を5000字以内にまとめてmarkdown形式で返す（重要な情報・結論・具体例を優先。必ず完全な文で終わること）\n"
    "    - 元記事が英語またはその他の言語の場合：日本語に翻訳したうえで、要点を残しながら2000字以内にまとめる（記事の論旨・重要な数値・結論を必ず含める）\n"
    "    - コードブロックは ```言語名 で囲んでそのまま保持（翻訳しない）\n"
    "    - インラインコード（関数名・変数名・ライブラリ名・スラッシュコマンド例: /clear, /resume）は `backtick` で囲んでそのまま保持\n"
    "    - 見出しは ## / ### / #### で構造を維持する（Setext形式のテキスト行の直後に --- や === が続く場合も ## / # に変換すること）\n"
    "    - 箇条書きは - で保持する\n"
    "    - 重要ワードは **太字** で保持する\n"
