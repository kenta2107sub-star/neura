...
def test_build_index_meta_counts_categories():
    ...
    meta = archive.build_index_meta("2026-06-18", arts)
...
def test_update_index_prepends_new_date():
    index = {"digests": [{"date": "2026-06-17", "count": 5, "categories": {}}]}
    meta = {"date": "2026-06-18", "count": 3, "categories": {"ニュース": 3}}
    out = archive.update_index(index, meta)
    assert out["digests"][0]["date"] == "2026-06-18"  # 先頭（降順維持）
    assert len(out["digests"]) == 2


def test_update_index_dedupes_same_date():
    index = {"digests": [{"date": "2026-06-18", "count": 1, "categories": {}}]}
    meta = {"date": "2026-06-18", "count": 7, "categories": {"ニュース": 7}}
    out = archive.update_index(index, meta)
    assert len(out["digests"]) == 1  # 重複日付は置換
