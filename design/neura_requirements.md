...
#### 処理フロー（2段階 Gemini 呼び出し）

**Stage 1: 記事選定**
1. 全記事のタイトル・冒頭700文字を `SELECTION_PROMPT` に含め、Gemini Flash API に送信する
2. Gemini が初心者向けに面白い記事を最大10件選び、URLのみをJSON配列で返す
3. 返却されたURLで元記事リストをフィルタし、選定記事を確定する
   - パース失敗・選定0件時：全件を Stage 2 へフォールバック

**Stage 2: 翻訳・要約**
4. 選定記事のタイトル・本文（最大3000文字）を `config.gemini_prompt` テンプレートに含め、Gemini Flash API（`gemini-2.5-flash`）に送信する
5. プロンプトの内容（`config.gemini_prompt` テンプレート）：
   ...
   - category: "ニュース" | "研究" | "活用事例" | "ツール"
   - importance: 1〜5（初心者が面白い・試してみたいと感じるかで判断）
6. Gemini APIのレスポンスをJSONとしてパースする
...
3. `PUT https://api.github.com/repos/{owner}/{repo}/contents/config/config.json` を fetch する
   ...
   - Authorization ヘッダーに `Bearer {PAT}` を付与する
4. `notify_schedules` が変更されていた場合、cron-job.org API で各スロットのジョブ時刻を更新する
   - `enabled: true` かつ `cron_job_id` が設定されているスロットを対象とする
   - cron-job.org API（`PATCH /jobs/{id}`）で `schedule.hours` を新しい JST 時刻に更新する
...
- リトライ：全エラーでリトライなし（次回の定時実行で再試行されるため）
