...
- Gemini APIへの個々のリクエストのタイムアウトは、処理内容に応じて分ける
  - Stage 1（タイトル＋冒頭700文字のみの軽量な選定処理）：30秒
  - Stage 2（本文最大5000文字×最大10件を読んで翻訳・要約・key_points生成まで行う重い処理）：120秒（実運用で30秒が短すぎて`504 DEADLINE_EXCEEDED`が発生し続けたため2026-07-23に120秒へ変更。障害の詳細はprogress.mdの決定事項を参照）
